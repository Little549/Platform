<template>
  <div class="shop">
    ...
    <!--绑定店铺列表-->
    <el-table :data="tableData" stripe style="width: 100%" height="480" size="mini">
      <el-table-column label="绑定店铺">
        <el-table-column prop="store_name" label="店铺名称" width="150"></el-table-column>
        <el-table-column prop="store_category" label="类目" width="80"></el-table-column>
        <el-table-column prop="fullAddress" label="发货地址" width="300"></el-table-column>
        <el-table-column prop="stateexpress" label="状态" width="100"></el-table-column>
    ...
    <template v-if="ifAdd">
      ...
      <!--添加店铺信息表单-->
      <el-row>
        <el-form class="shop-form" label-width="120px" ref="formMessage" :model="formMessage" :rules="rules" size="mini">
...
export default {
  ...
  methods: {
    ...
    getShopList () {
      var that = this
      this.$http.get('/store/?page=' + this.currentPage) // 将信息发送给后端
        .then((res) => { // axios返回的数据都在res.data里
          // console.log(res)
          if (res.status === 200) { // 如果成功
            ...
            for (var j = 0; j < this.tableData.length; j++) {
              if (this.tableData[j].state) {
                this.tableData[j].stateexpress = '店铺审核通过'
              } else {
                this.tableData[j].stateexpress = '店铺未审核'
              }
            }
          }
        })
